...
const LiveSession: React.FC<LiveSessionProps> = ({ mode, onClose, onReadSummary }) => {
  ...
  const handleRestart = () => {
    // Reset session state to restart
    setIsConnected(false);
    setHistory([]);
    historyRef.current = [];
    setRealtimeInput("");
    ...
    <div className="absolute inset-0 z-50 bg-black/95 backdrop-blur-xl flex animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative">
              ...
              <div className="p-8 text-center">
                ...
                <div className="bg-white/5 border-l-2 border-electric-gold p-6 text-left mb-8 rounded-r-lg">
                  <h3 className="text-electric-gold font-orbitron text-sm mb-3 flex items-center gap-2 uppercase tracking-wider">
                    <Zap className="w-4 h-4" /> Amazing Job!
                  </h3>
                  <p className="font-rajdhani text-lg text-gray-200 leading-relaxed mb-4">
                    You've successfully connected with {mode}. Keep exploring to strengthen your neural pathways!
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-3 w-full">
                  {onReadSummary && (
                    <button
                      ...
                      className="group w-full py-4 bg-purple-500/20 text-purple-300 border border-purple-500/50 font-orbitron font-bold tracking-[0.2em] hover:bg-purple-500 hover:text-white transition-all flex items-center justify-center gap-3"
                    >
                      <ClipboardCheck className="w-5 h-5" /> READ SUMMARY
                    </button>
                  )}

                  <button
                    onClick={handleRestart}
                    className="group w-full py-4 bg-white/10 text-white font-orbitron font-bold tracking-[0.2em] hover:bg-white/20 transition-all flex items-center justify-center gap-3"
                  >
                    <MessageSquare className="w-5 h-5" /> TALK AGAIN
                  </button>

                  <button
                    onClick={onClose}
                    className="group w-full py-4 bg-neon-blue/20 text-neon-blue border border-neon-blue font-orbitron font-bold tracking-[0.2em] hover:bg-neon-blue hover:text-black transition-all flex items-center justify-center gap-3"
                  >
                    RETURN TO MAP <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className={`relative w-[400px] h-[400px] flex items-center justify-center mb-8 transition-all duration-500 ${sessionStatus === 'summary' ? 'opacity-10 scale-90 blur-sm' : 'opacity-100'}`}>
          ...
          {generatedImage ? (
            ...
            <>
              ...
              <div
                className={`absolute inset-0 blur-[60px] rounded-full transition-all duration-75 ${mode === 'Synapse' ? 'bg-neon-blue' : 'bg-electric-gold'
                  }`}
                style={{
              ...
              {/* Central Core */}
              <div
                className={`relative z-20 w-32 h-32 rounded-full border-2 bg-black/80 backdrop-blur-xl flex items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-all duration-75 ${mode === 'Synapse' ? 'border-neon-blue shadow-neon-blue/20' : 'border-electric-gold shadow-electric-gold/20'
                  }`}
              ...
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className={`absolute rounded-full border transition-all duration-75 ease-out ${mode === 'Synapse' ? 'border-neon-blue' : 'border-electric-gold'
                    }`}
                  style={{
                    width: '100%',
                    height: '100%',
              ...
              {/* Spinning Data Ring */}
              <div className={`absolute w-[280px] h-[280px] rounded-full border border-dashed opacity-30 animate-[spin_10s_linear_infinite] ${mode === 'Synapse' ? 'border-neon-blue' : 'border-electric-gold'
                }`}></div>

              <div className={`absolute w-[320px] h-[320px] rounded-full border border-dotted opacity-20 animate-[spin_15s_linear_infinite_reverse] ${mode === 'Synapse' ? 'border-neon-blue' : 'border-electric-gold'
                }`}></div>
            </>
          )}
        ...
        <div className={`flex flex-col items-center gap-3 z-10 transition-all duration-500 ${sessionStatus === 'summary' ? 'opacity-10 blur-sm pointer-events-none' : 'opacity-100'}`}>
          <button
            onClick={toggleMic}
            className={`p-6 rounded-full transition-all duration-300 relative group ${isMicOn
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          {history.map((msg, idx) => (
            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[80%] p-3 rounded text-sm font-rajdhani leading-relaxed ${msg.role === 'user'
